...
class UltrafastLaneDetector():
	...
	def process_output(output, cfg):		
		...
		filtered_lanes_detected = [lanes_detected[i] for i in range(len(lanes_points)) if len(lanes_points[i]) > 0]

		# Pad the lists to ensure they have the same length
		max_length = max(len(lp) for lp in filtered_lanes_points)
		padded_lanes_points = [lp + [[0, 0]] * (max_length - len(lp)) for lp in filtered_lanes_points]

		print("filtered_lanes_points:", filtered_lanes_points)  # Debug: print filtered lanes_points
		print("filtered_lanes_detected:", filtered_lanes_detected)  # Debug: print filtered lanes_detected
		print("filtered_lanes_points shapes:", [len(lp) for lp in filtered_lanes_points])  # Debug: print shapes of filtered lanes_points
	...
	def draw_lanes(input_img, lanes_points, lanes_detected, cfg, draw_points=True):
		...
		if len(lanes_detected) > 2 and lanes_detected[1] and lanes_detected[2]:
